...
class AliyunInventory(object):
    ...
    def __init__(self):
        ...
        if self.args.host:
            host_vars = self.inventory['_meta']['hostvars'][self.args.host]
    ...
    def build_inventory(self):
        ...
        self.add_ecs(inventory)

        return inventory

    def push(self, inventory, key, element):
        ''' Push an element onto an array that may not have been defined in
        the dict '''
        group_info = inventory.setdefault(key, [])
    ...
    def add_ecs(self, inventory):
        for ecs in self.client.describe('ecs'):
            ecs = self.extract_ips(ecs)
            safe_name = ecs['PublicIp']
            self.push(inventory, 'ecs_instance_name_{}'.format(self.to_safe(ecs['InstanceName'])), safe_name)
            self.push(inventory, 'ecs', safe_name)
            for tag in ecs['Description'].split(','):
                tag = tag.strip()
                if len(tag) > 0:
                    self.push(inventory, 'tag_' + self.to_safe(tag), safe_name)

            ssh_options = self.ssh_options('ecs', safe_name, ecs)
            _ecs = ecs.copy()
            _ecs.update(ssh_options)
            inventory['_meta']['hostvars'][safe_name] = _ecs
